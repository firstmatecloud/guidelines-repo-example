...
export async function authenticateAPI(apiKey, clusterName) {

    const org = await organisationRepo.getOrganisationByApiKey(apiKey);
    if(!org){
    ...
    return async (req, res, next) => {
        try {
            ...
            if(requiredPermissions.includes(API_PERMISSIONS.PUBLIC_ENDPOINT)){
                ...
                const {cluster,org} = await authenticateAPI(iamObject.token, req.headers["cluster-name"]);
                iamObject.organisation = org
                iamObject.cluster = cluster
            }
            clsNamespace.run(() => {
